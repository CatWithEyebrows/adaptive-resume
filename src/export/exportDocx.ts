...
import type { ResumeData, VariantSection, WorkSection, Bullet } from "../types/resume";
 ...
 */
const getActiveBullets = (section: WorkSection): Bullet[] =>
 ...
 * @returns Array of DOCX Paragraphs.
 */
const buildWorkParagraphs = (section: WorkSection, isFirst: boolean): Paragraph[] => {
...
export const exportDocx = async (data: ResumeData): Promise<void> => {
  ...
  const workSections = data.sections.filter(
    (s): s is WorkSection => s.type === "work"
  );
